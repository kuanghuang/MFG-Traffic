function [F J] = eqs(tra, w)
    ...
    for j = 1 : Nx
        ...
        cnt = cnt + 1;
        u_min = uf * (1 - rho(j,n) / rhoj -...
                      uf * (V(r,n+1) - V(j,n+1))/dx);
        if u_min < 0
            F(cnt) = u(j,n);
            cntJ = cntJ + 1;
            Jrow(cntJ) = cnt;
        ...
        Jrow(cntJ) = cnt;
        Jcol(cntJ) = prho(j,n);
        Jval(cntJ) = u(j,n) / uf / rhoj;
        
        cntJ = cntJ + 1;
        Jrow(cntJ) = cnt;
        Jcol(cntJ) = pu(j,n);
        Jval(cntJ) = (u(j,n)/uf + rho(j,n)/rhoj - 1) / uf...
                   + (V(r,n+1) - V(j,n+1)) / dx;
    end
    end
